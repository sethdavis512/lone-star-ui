...
export const Default: Story = {
    render: (args) => (
        <div className="flex flex-col gap-1">
            <p className="text-sm font-medium text-pecan mb-1">Favorite Texas city</p>
...
export const Disabled: Story = {
    render: (args) => (
        <div className="flex flex-col gap-1">
            <p className="text-sm font-medium text-pecan/50 mb-1">Favorite Texas city</p>
            <RadioGroup defaultValue="austin" disabled {...args}>
                {texasOptions.map(({ value, label }) => (
                    <label key={value} className="flex items-center gap-2 text-sm text-pecan/50 cursor-not-allowed select-none">
                        <RadioRoot value={value}>
                            <RadioIndicator />
                        </RadioRoot>
                        {label}
...
export const Horizontal: Story = {
    render: (args) => (
        <div className="flex flex-col gap-1">
            ...
            <RadioGroup defaultValue="one-way" className="flex flex-row gap-4" {...args}>
                {[
